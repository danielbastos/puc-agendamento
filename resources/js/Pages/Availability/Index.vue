                        ...
                        <tr v-for="model in models" :key="model.id"
                            ...
                            <td class="px-6 py-4">
                                <span v-if="model.patient">
                                    <span v-if="isPatient && $page.props.auth.user.id != model.patient.id">Horário ocupado</span>
                                    <span v-if="!isPatient || $page.props.auth.user.id == model.patient.id">{{ model.patient.name }}</span>
                                </span>
                            ...
                            <td class="flex items-center px-6 py-4">
                                <CrudDeleteButton v-if="!model.past && model.patient && (!isPatient || $page.props.auth.user.id == model.patient.id)" :href="route('availability.schedule.cancel', model.id)" title="Cancelar" class="text-gray-900 bg-white border border-gray-300 focus:outline-none hover:bg-gray-100 focus:ring-4 focus:ring-gray-100 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 dark:bg-gray-800 dark:text-white dark:border-gray-600 dark:hover:bg-gray-700 dark:hover:border-gray-600 dark:focus:ring-gray-700">
                                    Cancelar
                                </CrudDeleteButton>
